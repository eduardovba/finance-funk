 ...
 */
export function validateBody<T>(schema: z.ZodSchema<T>, body: unknown): { data: T | null; error: string | null } {
    const result = schema.safeParse(body);
    if (!result.success) {
        const message = result.error.issues.map((i: z.ZodIssue) => `${i.path.join('.')}: ${i.message}`).join(', ');
 ...
 */
export function validateId(idString: string | null): { id: string | null; error: string | null } {
