...
async function runMigrations(): Promise<void> {
    ...
    for (const sql of migrations) {
        try { await db.execute(sql); } catch (_e) { /* already applied */ }
    }
    ...
    try { await db.execute('DROP TABLE IF EXISTS watchlist'); } catch (_e) { /* ignore */ }
    ...
    try {
        ...
        if (Number((hasUserIdCol.rows[0] as Record<string, unknown>).cnt) === 0) {
            ...
            } catch (_e) { /* snapshots table didn't exist yet, that's fine */ }
        ...
        if (adminUser.rows.length > 0) {
            ...
            for (const table of tables) {
                try {
                    ...
                    console.warn(`Skipped ${table} ownership migration:`, (tableErr as Error).message);
                }
            }
        }
    } catch (e) {
    ...
    try {
        ...
        if (duduRes.rows.length > 0) {
            const uid = (duduRes.rows[0] as Record<string, unknown>).id;

            // Helper to check if asset exists
            const assetExists = async (name: string): Promise<InValue> => {
                const r = await db.execute({ sql: "SELECT id FROM assets WHERE name = ? AND user_id = ?", args: [name, uid as InValue] });
                return r.rows.length > 0 ? (r.rows[0] as Record<string, unknown>).id as InValue : null;
            };

            // --- Andyara 1 (Sold) ---
            let a1Id = await assetExists('Andyara 1');
            if (!a1Id) {
                await db.execute({ sql: "INSERT INTO assets (name, asset_class, broker, currency, sync_status, user_id) VALUES (?, 'Real Estate', 'Manual', 'BRL', 'ACTIVE', ?)", args: ['Andyara 1', uid as InValue] });
                const a1 = await db.execute({ sql: "SELECT id FROM assets WHERE name = 'Andyara 1' AND user_id = ?", args: [uid as InValue] });
                a1Id = (a1.rows[0] as Record<string, unknown>).id as InValue;
                // Purchase, Tax, Sale entries
                await db.execute({ sql: "INSERT INTO ledger (date, type, asset_id, amount, price, currency, notes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", args: ['2015-01-01', 'Purchase', a1Id as InValue, 237000, 237000, 'BRL', 'Initial investment', uid as InValue] });
                await db.execute({ sql: "INSERT INTO ledger (date, type, asset_id, amount, price, currency, notes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", args: ['2023-01-01', 'Stamp Duty', a1Id as InValue, -9074, 0, 'BRL', 'Stamp Duty', uid as InValue] });
                await db.execute({ sql: "INSERT INTO ledger (date, type, asset_id, amount, price, currency, notes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", args: ['2023-06-01', 'Sale', a1Id as InValue, -360000, 360000, 'BRL', 'Property sold', uid as InValue] });
            ...
            if (!mcId) {
                await db.execute({ sql: "INSERT INTO assets (name, asset_class, broker, currency, sync_status, user_id) VALUES (?, 'Real Estate', 'Manual', 'BRL', 'ACTIVE', ?)", args: ['Rua Montes Claros', uid as InValue] });
                const mc = await db.execute({ sql: "SELECT id FROM assets WHERE name = 'Rua Montes Claros' AND user_id = ?", args: [uid as InValue] });
                mcId = (mc.rows[0] as Record<string, unknown>).id as InValue;
                await db.execute({ sql: "INSERT INTO ledger (date, type, asset_id, amount, price, currency, notes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", args: ['2018-01-01', 'Purchase', mcId as InValue, 681000, 681000, 'BRL', 'Initial investment', uid as InValue] });
                await db.execute({ sql: "INSERT INTO ledger (date, type, asset_id, amount, price, currency, notes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", args: ['2024-01-01', 'Stamp Duty', mcId as InValue, -29748, 0, 'BRL', 'Stamp Duty', uid as InValue] });
                await db.execute({ sql: "INSERT INTO ledger (date, type, asset_id, amount, price, currency, notes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", args: ['2024-06-01', 'Sale', mcId as InValue, -822920, 822920, 'BRL', 'Property sold', uid as InValue] });
            ...
            if (inkId) {
                ...
                const setupExists = await db.execute({ sql: "SELECT id FROM ledger WHERE asset_id = ? AND type = 'Mortgage Setup' AND user_id = ?", args: [inkId as InValue, uid as InValue] });
                if (setupExists.rows.length === 0) {
                    ...
                    await db.execute({ sql: "INSERT INTO ledger (date, type, asset_id, amount, price, currency, notes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", args: ['2024-03-01', 'Mortgage Setup', inkId as InValue, 0, 0, 'GBP', config, uid as InValue] });
                    console.log('Migration: Created Ink Court mortgage setup config.');
                }
                // Check if it has a valuation entry (price > 0)
                const hasValuation = await db.execute({ sql: "SELECT id FROM ledger WHERE asset_id = ? AND price > 0 AND user_id = ?", args: [inkId as InValue, uid as InValue] });
                if (hasValuation.rows.length === 0) {
                    await db.execute({ sql: "INSERT INTO ledger (date, type, asset_id, amount, price, currency, notes, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", args: ['2026-02-19', 'Valuation Update', inkId as InValue, 0, 620000, 'GBP', 'Initial valuation', uid as InValue] });
