...
export const getFixedIncomeSummary = (transactions, rates, endDate = null, assetClasses = {}) => {
    ...
    Object.values(accountMap).forEach(acc => {
        if (acc.syncValue !== null) {
            ...
            const toGBP = (val) => acc.nativeCurrency === 'GBP' ? val : val / (acc.nativeCurrency === 'BRL' ? rates.BRL : rates.USD);
    ...
    const totalGrossInv = assets.reduce((sum, a) => sum + a.grossInvGBP, 0);
    const totalRoi = totalInv !== 0 ? ((totalGbp - totalInv) / Math.abs(totalInv)) * 100 : 0;

    const individualHoldings = assets.map(a => ({
        name: a.name,
        brl: a.brl,
        gbp: a.gbp
    }));
...
export const getRealEstateSummary = (data = {}, marketData = {}, rates, endDate = null, assetClasses = {}) => {
    ...
    const propertyAssets = (properties || []).filter(p => p.status === 'Owned').map(p => {
        ...
        if (p.id === 'andyara-2') {
            ...
            const filteredLedger = endDate ? inkCourt.ledger.filter(l => parseDate(l.date) <= parseDate(endDate)) : inkCourt.ledger;
    ...
    const totalInv = activeAssets.reduce((sum, a) => sum + a.investmentGBP, 0);
    ...
    const individualHoldings = propertyAssets.map(a => ({
        name: a.name,
        brl: a.brl,
        gbp: a.gbp
    }));
...
export const getEquitySummary = (transactions, marketData = {}, rates, endDate = null, assetClasses = {}) => {
    ...
    const individualHoldings = activeHoldings.map(h => {
        const cur = BROKER_CURRENCY[h.broker] || 'GBP';
        let rawPrice = 0;
        let assetCurrency = cur;
