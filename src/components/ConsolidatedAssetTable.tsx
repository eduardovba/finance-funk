...
export default function ConsolidatedAssetTable({ title, categoryId, assets, rates, hideInvestment = false, onNavigate }: { title: string; categoryId?: string; assets: any[]; rates?: any; hideInvestment?: boolean; onNavigate?: (cat: string, name: string) => void }) {
    ...
    const formatSecondaryNoDecimals = (val: number) => formatSecondary(val, { minimumFractionDigits: 0, maximumFractionDigits: 0 });

    // assets should be an array of objects: { name, brl, gbp, investmentGBP, roi, isTotal }

    let activeAssets = assets.filter((a: any) => !a.isRealisedPnL && !a.isTotal && a.brl >= 10);

    // Hide entire section if no active holdings
    if (activeAssets.length === 0) return null;

    const realisedPnL = assets.find((a: any) => a.isRealisedPnL);
    const totalAsset = assets.find((a: any) => a.isTotal);

    // Calculate subtotal for active assets (Current Holdings)
    const subBrl = activeAssets.reduce((s: number, a: any) => s + (a.brl || 0), 0);
    const subGbp = activeAssets.reduce((s: number, a: any) => s + (a.gbp || 0), 0);
    const subInv = activeAssets.reduce((s: number, a: any) => s + (a.investmentGBP || 0), 0);
