...
let memoryCache: Record<string, { url: string | null; ts: number }> | null = null;
...
function getCache() {
    ...
    try {
        ...
        if (raw) {
            ...
            const now = Date.now();
            let pruned = false;
            // memoryCache is definitely an object here
            const validCache = memoryCache as Record<string, { url: string | null; ts: number }>;
            for (const key of Object.keys(validCache)) {
                if (now - (validCache[key].ts || 0) > CACHE_TTL_MS) {
                    delete validCache[key];
                    pruned = true;
                }
            }
            if (pruned) saveCache();
...
function getCachedLogo(ticker: string) {
...
function setCachedLogo(ticker: string, url: string | null) {
...
export default function AssetLogo({ ticker, name, size = 36, className = '' }: { ticker: string; name?: string; size?: number; className?: string }) {
