            ...
            >
                ...
                <div className="flex items-center justify-between gap-2 relative z-10 w-full">
                    <div className="flex flex-col min-w-0 pr-2">
                        <h3 className="text-[#F5F5DC]/60 text-[0.6875rem] xl:text-[0.75rem] tracking-[2px] uppercase font-space mb-0 truncate" title={title}>{title}</h3>
                        <p className={`text-2xl xl:text-3xl font-normal text-[#D4AF37] drop-shadow-[0_0_10px_rgba(212,175,55,0.4)] font-bebas truncate ${isLoading ? 'opacity-30' : 'opacity-100'}`}>
                            {isLoading ? '---' : <AnimatedNumber value={amount} formatter={(v) => formatCurrency(v, currency, { minimumFractionDigits: 0, maximumFractionDigits: 0 })} />}
                        </p>
                    </div>
                    <div className="flex flex-col items-end gap-2 flex-shrink-0">
                        <p className={`text-xs xl:text-sm font-mono tabular-nums text-[#CC5500] opacity-80 ${isLoading ? 'opacity-20' : ''} leading-none`}>
                            {isLoading ? '---' : formatCurrency(secondaryValue, secondaryCurrency, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                        </p>
                        <span className={`text-[0.75rem] font-mono tabular-nums leading-none ${isPositiveForColor ? 'text-vu-green' : 'text-red-400'}`}>
                            ...
                            <span className="text-[0.6875rem] opacity-70 ml-0.5">{isActuallyPositive ? '▲' : '▼'}</span>
                        </span>
                    </div>
                </div>
                ...
                <AnimatePresence mode="wait">
                    {isExpanded && contributors && contributors.length > 0 && (
                        ...
                        >
                            <div className="text-[0.6875rem] xl:text-[0.6875rem] text-parchment/30 uppercase tracking-widest mb-1">Top Contributors</div>
                            {contributors.map((c, idx) => (
                                <motion.div
                                    ...
                                    className="flex justify-between items-center text-[0.6875rem] xl:text-[0.75rem]"
                                >
                                    ...
                                    <div className="flex gap-2 font-mono tabular-nums">
                                        ...
                                        <span className="opacity-30 text-[0.6875rem] xl:text-[0.6875rem]">
        ...
        >
            ...
            <div className="mb-4 relative z-10">
                <div className="flex items-center justify-between gap-2">
                    <h3 className="text-[#F5F5DC]/60 text-[0.75rem] xl:text-xs mb-2 tracking-[2px] uppercase font-space truncate" title={title}>{title}</h3>
                ...
                <div className="flex justify-between items-center gap-2 flex-wrap">
                    <p className={`text-xs xl:text-sm font-mono tabular-nums text-[#CC5500] opacity-80 truncate ${isLoading ? 'opacity-20' : ''}`}>
                        {isLoading ? '---' : formatCurrency(secondaryValue, secondaryCurrency, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                    </p>
                    <div className={`
                        px-2 py-0.5 rounded-xl font-medium text-[0.75rem] flex items-center gap-1.5 leading-none shadow-sm
                        ${isPositiveForColor
                            ? 'text-vu-green bg-vu-green/[0.08] border border-vu-green/20'
                            : 'text-red-400 bg-red-400/[0.08] border border-red-400/20'
                        }
                    `}>
                        <span className="font-mono tabular-nums tracking-tight opacity-90 text-[0.6875rem]">
                            {(diffAmount || 0) > 0 ? '+' : ''}{formatCurrency(diffAmount || 0, primaryCurrency, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                        </span>
                        <div className="w-px h-2.5 bg-current opacity-30"></div>
                        <span className="flex items-center gap-0.5">
                            {Math.abs(percentage || 0).toFixed(1)}%
                            <span className="text-[0.6875rem] opacity-70 mb-[1px]">{isActuallyPositive ? '▲' : '▼'}</span>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <AnimatePresence mode="wait">
                {isExpanded && contributors && contributors.length > 0 && (
                    ...
                    >
                        <div className="text-[0.6875rem] text-parchment/30 uppercase tracking-widest mb-1">Top Contributors</div>
                        {contributors.map((c, idx) => (
                            <motion.div
                                key={idx}
                                initial={{ x: -5, opacity: 0 }}
                                animate={{ x: 0, opacity: 1 }}
                                transition={{ delay: idx * 0.05 + 0.1 }}
                                className="flex justify-between items-center text-[0.75rem]"
                            >
                                <span className="text-parchment/50 font-space truncate max-w-[120px]" title={c.name}>{c.name}</span>
                                <div className="flex gap-2 font-mono tabular-nums">
                                    <span className={c.amount >= 0 ? 'text-vu-green/90' : 'text-red-400/90'}>
                                    ...
                                    <span className="opacity-30 text-[0.6875rem]">
                                        ({c.percentage >= 0 ? '+' : ''}{c.percentage.toFixed(1)}%)
                                    </span>
                                </div>
                            </motion.div>
