...
function ChartListItem({ item, updateItem, removeItem, isExpanded, setExpandedId }) {
...
export default function DashboardCustomizer({ initialConfig, onClose, onSave }) {
    const [charts, setCharts] = useState(initialConfig?.charts || []);
    const [expandedId, setExpandedId] = useState(null);
    const [isSaving, setIsSaving] = useState(false);

    // Filter valid charts just in case
    const safeCharts = charts.filter(c => c && c.id);

    const handleSave = async () => {
        setIsSaving(true);
        try {
            const updatedConfig = { ...initialConfig, charts: safeCharts.map((c, i) => ({ ...c, order: i })) };
