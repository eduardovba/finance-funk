                            ...
                            <tbody>
                                {data.transactions.map((tr, idx) => (
                                    <tr key={tr.id || idx} className="border-b border-white/[0.03] hover:bg-white/[0.03] transition-colors cursor-pointer"
                                        ...
                                        <td className="px-4 py-3 text-white/70 font-space  text-data-xs">{tr.date}</td>
                                        <td className="px-4 py-3 text-right text-rose-400 font-bold font-space tabular-nums">{formatCurrency(convertCurrency(tr.value_brl || 0, 'BRL', lenderCur, rates as any), lenderCur)}</td>
                                        <td className="px-4 py-3 text-right text-white/60 font-space tabular-nums hidden sm:table-cell">{lenderCur !== 'BRL' ? formatCurrency(tr.value_brl, 'BRL') : '—'}</td>
