            ...
            >
                ...
                <div className="flex items-center justify-between gap-2 relative z-10 w-full">
                    <div className="flex flex-col min-w-0 pr-2">
                        <div className="flex items-center gap-2 mb-0 min-w-0">
                            <h3 className="text-[#F5F5DC]/60 text-2xs xl:text-xs tracking-[2px] uppercase font-space truncate" title={typeof title === 'string' ? title : undefined}>
                                {title}
                            </h3>
                            {badge && <span className="px-1.5 py-0.5 rounded-full bg-white/10 text-white/70 tracking-normal normal-case text-[0.6rem] whitespace-nowrap shrink-0">{badge}</span>}
                        </div>
                    ...
                    <div className="flex flex-col items-end gap-2 flex-shrink-0">
                        ...
                        <span className={`text-data-xs font-space  leading-none ${isPositiveForColor ? 'text-vu-green' : 'text-red-400'}`}>
                            ...
                            <span className="text-2xs opacity-70 ml-0.5">{isActuallyPositive ? '▲' : '▼'}</span>
                        </span>
                    </div>
                </div>
                ...
                <AnimatePresence mode="wait">
                    {isExpanded && contributors && contributors.length > 0 && (
                        ...
                        >
                            <div className="text-2xs xl:text-2xs text-parchment/30 uppercase tracking-widest mb-1">Top Contributors</div>
                            {contributors.map((c: any, idx: number) => (
                                <motion.div
                                    ...
                                    className="flex justify-between items-center text-2xs xl:text-xs"
                                >
                                    <span className="text-parchment/50 font-space truncate max-w-[120px]" title={c.name}>{c.name}</span>
                                    <div className="flex gap-2 font-space tabular-nums">
                                        ...
                                        <span className="opacity-30 text-2xs xl:text-2xs">
        ...
        >
            ...
            <div className="mb-4 relative z-10">
                <div className="flex items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2 min-w-0">
                        <h3 className="text-[#F5F5DC]/60 text-xs xl:text-xs tracking-[2px] uppercase font-space truncate" title={typeof title === 'string' ? title : undefined}>
                            {title}
                        </h3>
                        {badge && <span className="px-1.5 py-0.5 rounded-full bg-white/10 text-white/70 tracking-normal normal-case text-[0.65rem] whitespace-nowrap shrink-0">{badge}</span>}
                    </div>
            ...
            <AnimatePresence mode="wait">
                {isExpanded && contributors && contributors.length > 0 && (
                    ...
                    >
                        <div className="text-2xs text-parchment/30 uppercase tracking-widest mb-1">Top Contributors</div>
                        {contributors.map((c: any, idx: number) => (
                            <motion.div
                                key={idx}
                                initial={{ x: -5, opacity: 0 }}
                            ...
                            >
                                ...
                                <div className="flex gap-2 font-space tabular-nums">
                                    ...
                                    <span className="opacity-30 text-2xs">
                                        ({c.percentage >= 0 ? '+' : ''}{c.percentage.toFixed(1)}%)
                                    </span>
                                </div>
                            </motion.div>
