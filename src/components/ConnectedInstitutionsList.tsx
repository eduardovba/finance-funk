...
export default function ConnectedInstitutionsList() {
    ...
    // Phase 3: Review Flow
    const [reviewModal, setReviewModal] = useState<{isOpen: boolean, assets: any[], institution: string}>({ isOpen: false, assets: [], institution: '' });
    ...
    return (
        <div className="space-y-4">
            {Array.isArray(connections) && connections.map(conn => {
                const pendingAssets = conn.assets?.filter((a: any) => a.sync_status === 'PENDING') || [];
                const activeAssets = conn.assets?.filter((a: any) => a.sync_status === 'ACTIVE') || [];
