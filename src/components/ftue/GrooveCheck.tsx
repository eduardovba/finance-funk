...
export default function GrooveCheck() {
    ...
    useEffect(() => {
        ...
        if ((goal === 'budget' || goal === 'both') && budgetTransactions?.length > 0 && budgetCategories?.length > 0) {
            ...
            const recentTxns = budgetTransactions.filter(t => !incomeCatIds.has(t.category_id as number) && new Date(t.date) >= weekAgo);
            ...
            if (recentTxns.length > 0) {
                ...
                recentTxns.forEach(t => {
                    catTotals[t.category_id as number] = (catTotals[t.category_id as number] || 0) + (t.amount_cents || 0);
                });
                const topCatId = Object.entries(catTotals).sort((a: any, b: any) => b[1] - a[1])[0]?.[0];
