 ...
 */
export default function PageTutorialOverlay({ pageId, steps = [] }: any) {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            let available = steps.filter((s: any) => document.getElementById(s.targetId));
    ...
    const getTooltipStyle = (): any => {
        ...
        const flipMap: any = { top: 'bottom', bottom: 'top', left: 'right', right: 'left', 'bottom-end': 'top' };

        // Try preferred placement first
        let { top, left } = calcPos(preferred);

        // Check if it clips and auto-flip if needed
        const clips = (t: number, l: number) =>
            t < VIEWPORT_MARGIN ||
