...
// --- Shared Tooltip ---
const PodTooltip = ({ active, payload, label, isCurrency = true }: any) => {
    ...
    return (
        <div className="rounded-xl" style={{ background: 'rgba(18, 20, 24, 0.90)', border: '1px solid rgba(255,255,255,0.06)', padding: '10px 14px', backdropFilter: 'blur(12px)', boxShadow: '0 8px 32px rgba(0,0,0,0.5)', }}>
            ...
            {payload.filter((e: any) => e.value != null && e.value !== 0).map((entry: any, i: number) => (
...
// --- Pod Card Wrapper ---
const PodCard = ({ title, icon, children }: any) => (
...
export default function LedgerHeroPods({ snapshots, forecastActuals, targetROI, targetContribution }: any) {
    ...
    const yearlyROIData = useMemo(() => {
        ...
        const yearlyContribs: { [key: string]: number } = {};
        if (forecastActuals && forecastActuals.length) {
            forecastActuals.forEach((d: any) => {
                if (!d.date) return;
        ...
        return years.map((year: string) => {
            const monthsInYear = yearMap[year].sort((a: any, b: any) => a.month.localeCompare(b.month));
        ...
        }).filter((d: any) => d.year >= '2022'); // Only show meaningful years
    ...
    const yearlyContribData = useMemo(() => {
        ...
        return Object.keys(yearMap).sort().map((year: string) => ({
            year,
                        ...
                        <AreaChart data={compositionData}>
                            ...
                            {['FixedIncome', 'Equity', 'RealEstate', 'Crypto', 'Pensions'].map(key => (
                                <Area
                                    ...
                                    stroke={(CATEGORY_COLORS as any)[key]}
                                    strokeWidth={0.5}
                                />
                            ))}
                        </AreaChart>
                    ...
                    <ResponsiveContainer>
                        <BarChart data={yearlyROIData} barCategoryGap="20%">
                            ...
                            <Bar dataKey="actual" name="Actual" radius={[4, 4, 0, 0]} barSize={20}>
                                {yearlyROIData.map((entry: any, index: number) => (
                                    <Cell
                                        key={index}
                ...
                <div className="w-full" style={{ height: '220px' }}>
                    <ResponsiveContainer>
                        <BarChart data={yearlyContribData} barCategoryGap="20%">
                            ...
                            <Bar dataKey="actual" name="Actual" radius={[4, 4, 0, 0]} barSize={20}>
                                {yearlyContribData.map((entry: any, index: number) => (
                                    <Cell
                                        key={index}
