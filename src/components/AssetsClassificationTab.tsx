...
export default function AssetsClassificationTab({
    ...
    realEstate
}: { assetClasses?: any; onSave: (overrides: any) => Promise<void>; equityTransactions?: any[]; cryptoTransactions?: any[]; pensionTransactions?: any[]; debtTransactions?: any[]; transactions?: any[]; realEstate?: any; }) {
    ...
    const allAssets = useMemo(() => {
        ...
        const addAsset = (name: string, defaultCategory: string, defaultCurrency = 'GBP', sourceType: string | null = null, broker: string | null = null) => {
            if (!name) return;
        ...
        const brokerCashSeen = new Set<string>();
        equityTransactions?.forEach((tr: any) => {
        ...
        // 2. Crypto
        cryptoTransactions?.forEach((tr: any) => {
        ...
        pensionTransactions?.forEach((tr: any) => {
            let allocationCat = tr.allocationClass || 'Equity';
            if (tr.asset === 'Cash') {
                allocationCat = 'Cash';
            } else {
                const mapEntry = (pensionMap as any[]).find(m => m.asset === tr.asset);
                if (mapEntry && mapEntry.allocations) {
                    let topCat = null;
                    let topVal = -1;
                    for (const [cat, val] of Object.entries<number>(mapEntry.allocations)) {
        ...
        debtTransactions?.forEach((tr: any) => {
        ...
        if (realEstate?.properties) {
            realEstate.properties.forEach((p: any) => {
        ...
        if (realEstate?.funds?.holdings) {
            ...
            realEstate.funds.holdings.forEach((h: any) => {
    ...
    const handleOverrideChange = (assetName: string, field: string, value: string) => {
                                ...
                                <div className="space-y-3">
                                    <div className="flex flex-col gap-1.5">
                                        <div className="flex justify-between items-center">
                                            ...
                                            {isCatOverridden && (
                                                <span className="text-xs bg-emerald-500/20 text-emerald-400 px-1.5 py-0.5 rounded font-semibold tracking-wide uppercase">
                                                    Overridden
                                                </span>
                                            )}
                                        </div>
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <div className="flex justify-between items-center">
                                            ...
                                            {isCurOverridden && (
                                                <span className="text-xs bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded font-semibold tracking-wide uppercase">
                                                    Overridden
                                                </span>
                                            )}
                                        </div>
