...
export default function TransactionLedger({
    ...
    const headerStyle: React.CSSProperties = { padding: '16px', color: 'var(--fg-secondary)', cursor: 'pointer', userSelect: 'none' };
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    ...
                    {collapsible && (
                        <button
                            ...
                            onMouseOver={(e: any) => e.currentTarget.style.color = 'var(--fg-primary)'}
                            onMouseOut={(e: any) => e.currentTarget.style.color = 'var(--fg-secondary)'}
                        >
                            {isOpen ? 'Hide' : 'Show'}
            ...
            {isOpen && (
                <div className="rounded-2xl bg-[#121418]/60 backdrop-blur-xl border border-white/[0.06] shadow-[0_8px_32px_rgba(0,0,0,0.4)]" style={{ padding: '0', overflow: 'hidden' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }} className="ledger-table">
                        ...
                        <tbody>
                            {sortedTransactions.map((tr: any, index: number) => {
