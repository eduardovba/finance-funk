...
export default function MonthlyCloseModal({
    ...
    const snapshotData = useMemo(() => {
        const fi = getFixedIncomeSummary(rawFixedIncome, rates, endDate as any, assetClasses);
        const eq = getEquitySummary(rawEquity, marketData, rates, endDate as any, assetClasses);
        const cr = getCryptoSummary(rawCrypto, marketData, rates, endDate as any, assetClasses);
        const pn = getPensionSummary(rawPensions, rates, pensionPrices, marketData, endDate as any, assetClasses);
        ...
        const filteredHistoricalIncome = (ledgerData?.income || []).filter((h: any) => h.month !== selectedMonth);
        const filteredHistoricalInvestments = (ledgerData?.investments || []).filter((h: any) => h.month !== selectedMonth);
        ...
        const monthIncome = combinedIncome.find((d: any) => d.month === selectedMonth) || { total: 0, salary: 0, realEstate: 0, equity: 0, fixedIncome: 0 };
        const monthInvest = monthlyInv.find((d: any) => d.month === selectedMonth) || { total: 0, equity: 0, fixedIncome: 0, realEstate: 0, pensions: 0, crypto: 0, debt: 0 };

        // MoM Variation
        const sorted = [...historicalSnapshots].sort((a, b) => a.month.localeCompare(b.month));
        const prevSnapshot = sorted.filter((s: any) => s.month < selectedMonth).pop();
        ...
        return {
            ...
            assetDetails: {
                'fixed-income': fi.individualHoldings || fi.assets.filter((a: any) => !a.isTotal).map((a: any) => ({ name: a.name, brl: a.brl, gbp: a.gbp })),
                'equity': eq.individualHoldings || eq.assets.filter((a: any) => !a.isTotal).map((a: any) => ({ name: a.name, brl: a.brl, gbp: a.gbp })),
                'real-estate': re.individualHoldings || re.assets.filter((a: any) => !a.isTotal).map((a: any) => ({ name: a.name || 'Real Estate', brl: a.brl, gbp: a.gbp })),
                'crypto': cr.individualHoldings || cr.assets.filter((a: any) => !a.isTotal).map((a: any) => ({ name: a.name, brl: a.brl, gbp: a.gbp })),
                                    ...
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            ...
                                            <div className="mt-1 flex items-center gap-1.5">
                                                <span className="text-xs" style={{ fontWeight: '700', color: ((snapshotData.categoryDiffs as any)?.[cat.id] || 0) >= 0 ? 'var(--vu-green)' : 'var(--error)' }}>
                                                    {((snapshotData.categoryDiffs as any)?.[cat.id] || 0) >= 0 ? '+' : ''}
                                                    {(snapshotData.categoryPercs as any)?.[cat.id]?.toFixed(1)}%
                                                </span>
                                                <span className="text-[0.65rem] opacity-70" style={{ color: 'var(--fg-secondary)' }}>
                                                    {((snapshotData.categoryDiffs as any)?.[cat.id] || 0) >= 0 ? '+' : '-'}
                                                    {primaryMeta?.symbol}{Math.abs(toPrimary((snapshotData.categoryDiffs as any)?.[cat.id] || 0, 'BRL')).toLocaleString(primaryMeta?.locale, { maximumFractionDigits: 0 })}
                                                </span>
                                            </div>
                                        </div>
                                        <span className="tracking-[0.5px]" style={{ fontSize: '1.2rem', fontWeight: '800', fontFamily: 'var(--font-bebas)' }}>
                                            {primaryMeta?.symbol}{((snapshotData.categories as any)?.[cat.id] || 0).toLocaleString(primaryMeta?.locale, { maximumFractionDigits: 0 })}
                                        </span>
                                    </div>
