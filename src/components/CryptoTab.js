...
export default function CryptoTab({ transactions = [], marketData, rates, onRefresh }) {
    ...
    const handleSellConfirm = async () => {
        ...
        const tr = {
            ...
            investment: -price * Math.abs(qty), // Proceeds are negative investment
            quantity: -Math.abs(qty), // Negative quantity for sell
            pnl: parseFloat(sellData.pnl) || 0, // Explicit P&L entered by user
            type: 'Sell',
            platform: sellData.broker
        };
        try {
            ...
            alert('Sell transaction failed: ' + e.message);
            console.error(e);
        }
    };
    ...
    const handleBuyConfirm = async () => {
        ...
        const tr = {
            ...
            investment: price * qty, // Total investment amount (what was actually paid)
            quantity: qty,
            isSalaryContribution: buyData.isSalaryContribution || false,
            type: 'Buy',
            platform: buyData.broker
        };
        try {
            const res = await fetch('/api/crypto-transactions', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    const computeHoldings = () => {
        ...
        sorted.forEach(tr => {
            ...
            // Track explicit P&L from transactions that have it
