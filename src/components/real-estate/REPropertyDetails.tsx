                                ...
                                renderItem={(tx: any) => (
                                    <>
                                        <div className="flex items-center gap-2 mb-1.5">
                                            ...
                                            <span className="font-medium text-[10px] text-white/90 uppercase tracking-wider font-space">{tx.type || tx.notes || 'Transaction'}</span>
                                        </div>
                                        <div className="flex flex-col gap-1">
                                            <span className="text-sm font-bold text-white tracking-tight font-mono">{formatCurrency(Math.abs(tx.amount), asset.currency)}</span>
                                            <span className="text-[10px] text-white/40 font-mono tracking-tight">{tx.date}</span>
                                        </div>
                                    </>
                                )} />
                        </div>
                        ...
                        <TransactionTimeline transactions={mortgagePayments.slice(0, 20)}
                            ...
                            renderItem={(tx: any) => (
                                <>
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <span className="text-sm font-bold text-white tracking-tight font-mono">{formatCurrency(tx.costs, asset.currency)}</span>
                                        <span className="text-[10px] text-white/40 font-mono tracking-tight">Principal: {formatCurrency(tx.principal, asset.currency)}</span>
                                    </div>
                                </>
                            )} />
                    </div>
                                ...
                                renderItem={(tx: any) => (
                                    <>
                                        <div className="flex items-center gap-2 mb-1.5">
                                            ...
                                            <span className="font-medium text-[10px] text-white/90 uppercase tracking-wider font-space">{tx.type === 'Income' ? 'Revenue' : 'Cost'}</span>
                                        </div>
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <span className="text-[10px] text-white/40 font-mono tracking-tight">{tx.date}{tx.notes ? ` · ${tx.notes}` : ''}</span>
                                        </div>
                                    </>
                                )} />
                        </div>
                    ...
                    return (
                        <div className="p-3 rounded-lg bg-black/30 border border-white/5">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className={`text-sm font-bold font-mono ${profit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{profit >= 0 ? '+' : ''}{formatCurrency(profit, asset.currency)}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-[10px] text-white/40 uppercase tracking-widest">ROI</span>
                                <span className={`text-sm font-bold font-mono ${roiCalc >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{roiCalc >= 0 ? '+' : ''}{roiCalc.toFixed(1)}%</span>
                            </div>
                        </div>
                    );
                })()}
