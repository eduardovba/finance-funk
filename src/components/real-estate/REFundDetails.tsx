        ...
        <>
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/[0.02] border border-white/5 rounded-xl p-3">
                    <span className="block text-xs text-white/40 uppercase tracking-widest mb-1.5">Shares</span>
                    <span className="text-data-sm font-medium text-white/90 font-space ">{asset.totalQuantity}</span>
                </div>
                <div className="bg-white/[0.02] border border-white/5 rounded-xl p-3">
                    <span className="block text-xs text-white/40 uppercase tracking-widest mb-1.5">Live Price</span>
                    <span className="text-data-sm font-medium text-white/90 font-space ">{formatCurrency(asset.currentPrice, 'BRL')}</span>
                </div>
                <div className="bg-white/[0.02] border border-white/5 rounded-xl p-3">
                    <span className="block text-xs text-white/40 uppercase tracking-widest mb-1.5">Avg Cost</span>
                    <span className="text-data-sm font-medium text-white/90 font-space ">{formatCurrency(avgCost, 'BRL')}</span>
                </div>
                <div className="bg-[#D4AF37]/5 border border-[#D4AF37]/20 rounded-xl p-3">
                    ...
                    <span className="text-data-sm font-bold text-[#D4AF37] font-space ">{formatCurrency(asset.currentValue, 'BRL')}</span>
                </div>
                <div className="col-span-2 bg-white/[0.02] border border-white/5 rounded-xl p-3 flex justify-between items-center">
                    ...
                    <span className={`text-data-sm font-bold font-space  ${asset.pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
            ...
            {asset.transactions && asset.transactions.length > 0 && (
                <div className="pt-4 border-t border-white/5 mt-4">
                    <h4 className="text-xs text-white/40 uppercase tracking-[2px] mb-3">Transactions</h4>
                            ...
                            renderItem={(tx: any) => (
                                <>
                                    <div className="flex items-center gap-2 mb-1.5">
                                        ...
                                        <span className="font-medium text-xs text-white/90 uppercase tracking-wider font-space">{tx.quantity >= 0 ? 'Bought' : 'Sold'}</span>
                                    </div>
                                    <div className="flex flex-col gap-1">
                                        <span className="text-data-sm font-bold text-white tracking-tight font-space ">{formatCurrency(Math.abs(tx.investment), 'BRL')}</span>
                                        <span className="text-data-xs text-white/40 font-space  tracking-tight">
