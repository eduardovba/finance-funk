...
function DemoAppShellInner({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        if (demoData.budgetTransactions) {
            const txns = demoData.budgetTransactions.map((t: any) => ({ ...t, currency: userCurrency }));
            ...
            const filtered = txns.filter((t: any) => t.date.startsWith(cm));
            store.setTransactions(filtered);
        }
        if (demoData.budgetRollups) {
            const currentMonth = `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}`;
            const currentRollup = demoData.budgetRollups.find((r: any) => r.month === currentMonth) || demoData.budgetRollups[0];
