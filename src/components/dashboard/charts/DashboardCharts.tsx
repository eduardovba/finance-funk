...
import {
    ResponsiveContainer, ComposedChart, XAxis, YAxis, CartesianGrid, Tooltip, Area,
    BarChart, Bar, Legend, ReferenceLine, PieChart, Pie, Cell, LineChart, Line, LabelList
} from 'recharts';
...
export default function DashboardCharts({ historicalData, currentMonthData, rates, monthlyInvestments, masterMixData, allocationTargets, forecastSettings, dashboardConfig, onCustomizeClick, onNavigate }: DashboardChartsProps) {
    ...
    const twrHistoryMap: any = calculateTWRHistory(snapshotsForTWR, monthlyInvestments || [], rates);

    const anchorMode = forecastSettings?.anchorMode || 'historical';
    const firstActualValueBrl = (actualsData as any)[0]?.actualBRL || 0;
    const firstDateObj = parseDate((actualsData as any)[0]?.date);
    ...
    while (nextDate <= targetDate) {
        ...
        futureData.push({
            month: `${yyyy}-${mm}`,
            networthBRL: null, networthPrimary: null, networthSecondary: null,
            roi: null, actuals: {}, categories: {}
        });
        nextDate.setMonth(nextDate.getMonth() + 1);
    }

    const displayDataRaw = [
        ...data.filter((d: any) => d.month !== currentMonth).map((d: any) => {
            const nwBrl = d.networthBRL || d.totalminuspensionsBRL || 0;
            const nwGbp = d.networthGBP || d.totalminuspensionsGBP || (nwBrl / rates.BRL);

            const mapHistorical = (cur: string) => {
                if (cur === 'BRL') return nwBrl;
                if (cur === 'GBP') return nwGbp;
                return toPrimary(nwBrl, 'BRL');
            };
        ...
        }),
        {
            ...
            impliedRate: (rates[primaryCurrency] || 1) / (rates[secondaryCurrency] || 1),
            actuals: {
                FixedIncome: toPrimary(currentMonthData.summaries.find((s: any) => s.id === 'fixed-income')?.amount || 0, 'BRL'),
                Equity: toPrimary(currentMonthData.summaries.find((s: any) => s.id === 'equity')?.amount || 0, 'BRL'),
                RealEstate: toPrimary(currentMonthData.summaries.find((s: any) => s.id === 'real-estate')?.amount || 0, 'BRL'),
                Crypto: toPrimary(currentMonthData.summaries.find((s: any) => s.id === 'crypto')?.amount || 0, 'BRL'),
                Pensions: toPrimary(currentMonthData.summaries.find((s: any) => s.id === 'pensions')?.amount || 0, 'BRL'),
                Debt: toPrimary(currentMonthData.summaries.find((s: any) => s.id === 'debt')?.amount || 0, 'BRL')
            },
            categories: (() => {
                const fi = currentMonthData.summaries.find((s: any) => s.id === 'fixed-income')?.amount || 0;
                const eq = currentMonthData.summaries.find((s: any) => s.id === 'equity')?.amount || 0;
                const re = currentMonthData.summaries.find((s: any) => s.id === 'real-estate')?.amount || 0;
                const cr = currentMonthData.summaries.find((s: any) => s.id === 'crypto')?.amount || 0;
                const pe = currentMonthData.summaries.find((s: any) => s.id === 'pensions')?.amount || 0;
                const de = currentMonthData.summaries.find((s: any) => s.id === 'debt')?.amount || 0;
    ...
    const displayData = displayDataRaw.map((pt: any, i: number, arr: any[]) => {
        ...
        return { ...pt, actualGreen, actualRed };
    });

    const historicalDisplayData = displayData.filter((d: any) => d.networthPrimary !== null);

    const investmentsDataMap = new Map();
    if (monthlyInvestments) {
        monthlyInvestments.forEach((d: any) => investmentsDataMap.set(d.month, d));
    }
    const investmentsData: any[] = [];
    const sortedMonths = monthlyInvestments ? [...monthlyInvestments].map((d: any) => d.month).sort() : [];
    const firstMonthData = sortedMonths.length > 0 ? sortedMonths[0] : currentMonth;
    let cY = parseInt(firstMonthData.split('-')[0]);
    let cM = parseInt(firstMonthData.split('-')[1]);
    const endY = parseInt(currentMonth.split('-')[0]);
    const endM = parseInt(currentMonth.split('-')[1]);
    while (cY < endY || (cY === endY && cM <= endM)) {
        const mmStr = String(cM).padStart(2, '0');
        const monthStr = `${cY}-${mmStr}`;
        const d: any = investmentsDataMap.get(monthStr) || {};
        const netFlowRaw = d.total !== undefined ? d.total : ((d.equity || 0) + (d.fixedIncome || 0) + (d.realEstate || 0) + (d.crypto || 0) + (d.pensions || 0) + (d.debt || 0));
        investmentsData.push({ month: monthStr, Net: netFlowRaw });
        cM++;
        if (cM > 12) { cM = 1; cY++; }
    }

    const currencyTotals: any = { Primary: 0, Secondary: 0, Other: 0 };
    if (currentMonthData && currentMonthData.categories) {
        currentMonthData.categories.forEach((cat: any) => {
            if (cat.assets) {
                cat.assets.forEach((asset: any) => {
                    if (!asset.isTotal && !asset.isRealisedPnL && asset.name !== 'Total') {
                        ...
                        if (cur === primaryCurrency) currencyTotals.Primary += val;
                        else if (cur === secondaryCurrency) currencyTotals.Secondary += val;
                        else currencyTotals.Other += val;
                    }
                });
            }
        });
    ...
    const actuals: any = masterMixData?.percentages || { Equity: 0, FixedIncome: 0, RealEstate: 0, Crypto: 0, Cash: 0 };
    const rawTargets: any = allocationTargets?.assetClasses || allocationTargets || { Equity: 50, FixedIncome: 30, RealEstate: 15, Crypto: 5, Cash: 0 };
    ...
    const firstDP = displayData.find((d: any) => d.networthPrimary > 0 && d.networthSecondary > 0);
    let domainPrimary: any = ['auto', 'auto'];
    let domainSecondary: any = ['auto', 'auto'];

    if (firstDP) {
        const v1_start = firstDP.networthPrimary;
        const v2_start = firstDP.networthSecondary;
        const v1_all = displayData.map((d: any) => d.networthPrimary).filter((v: any) => v > 0);
        const v2_all = displayData.map((d: any) => d.networthSecondary).filter((v: any) => v > 0);
    ...
    const dataRegistry: any = {
        'networth-history': historicalDisplayData,
        'category-history': historicalDisplayData,
        'allocation-current': allocationData,
        'roi-history': displayData.filter((d: any) => d.roi !== null || (d.impliedRate !== null && d.impliedRate > 0)),
        'fx-rate-history': displayData.filter((d: any) => d.roi !== null || (d.impliedRate !== null && d.impliedRate > 0)),
    ...
    return (
        <>
            <div className="grid grid-cols-1 md:grid-cols-[repeat(auto-fit,minmax(500px,1fr))] gap-4 md:gap-6 mb-8">
                {chartsToRender.map((cfg: any, index: number) => (
...
function GenericChart({ config, dataRegistry, meta, onCustomizeClick, onNavigate }: any) {
    ...
    const renderChart = () => {
        ...
        if (chartType === 'stacked-bar' || chartType === 'stacked-area') {
            const COMP_COLORS: any = { FixedIncome: '#10b981', Equity: '#3b82f6', RealEstate: '#ef4444', Crypto: '#f59e0b', Pensions: '#8b5cf6', Debt: '#ec4899' };
            const COMP_LABELS: any = { FixedIncome: 'Fixed Income', Equity: 'Equity', RealEstate: 'Real Estate', Crypto: 'Crypto', Pensions: 'Pensions', Debt: 'Debt' };
            const areaKeys = ['FixedIncome', 'Equity', 'RealEstate', 'Crypto', 'Pensions'].filter(k => series.includes(k));
            const lastDataPoint: any = data.length > 0 ? data[data.length - 1] : {};
            areaKeys.sort((a, b) => {
            ...
            const chartData = showDebt ? (() => {
                ...
                const firstDebtIdx = mapped.findIndex((d: any) => d.grossAssets !== null);
                if (firstDebtIdx > 0) {
            ...
            return (
                <ComposedChart data={chartData}>
                    <defs>
                        {Object.entries(COMP_COLORS).map(([key, color]) => (
                            <linearGradient key={key} id={`comp-grad-dash-${key}`} x1="0" y1="0" x2="0" y2="1">
                                <stop offset="0%" stopColor={color as string} stopOpacity={0.7} />
                                <stop offset="100%" stopColor={color as string} stopOpacity={0.15} />
                            </linearGradient>
                        ))}
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" vertical={false} />
                    <XAxis dataKey="month" stroke="transparent" tick={{ fill: 'rgba(245,245,220,0.3)', fontSize: 10, fontFamily: 'var(--font-space)' }} tickFormatter={formatMonthYear} tickLine={false} axisLine={false} interval={"preserveStartEnd"} minTickGap={30} />
                    <YAxis stroke="transparent" tick={{ fill: 'rgba(245,245,220,0.25)', fontSize: 10, fontFamily: 'var(--font-space)' }} tickFormatter={(val: number) => {
                    ...
                    <Tooltip content={<CustomTooltip {...customTooltipProps} />} />
                    {areaKeys.map(key => (
                        <Area key={key} type="monotone" dataKey={`categories.${key}`} name={COMP_LABELS[key]} stackId="1" fill={`url(#comp-grad-dash-${key})`} stroke={COMP_COLORS[key]} strokeWidth={0.5} />
                    ))}
                    {showDebt && <Line type="monotone" dataKey="grossAssets" name="Gross Assets (excl. Debt)" stroke="#ec4899" strokeWidth={1.5} strokeDasharray="6 3" dot={false} connectNulls />}
                </ComposedChart>
            );
        }
            ...
            return (
                <BarChart data={data} layout="vertical" margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                    ...
                    {series.includes('target') && (
                        <Bar dataKey="target" name="Target %" fill="rgba(245, 245, 220, 0.2)" stroke="#F5F5DC" strokeWidth={1} strokeDasharray="2 2" radius={[0, 4, 4, 0] as any} barSize={16}>
                            <LabelList dataKey="target" position="right" formatter={(val: number) => `${val.toFixed(1)}%`} style={{ fill: '#F5F5DC', fontSize: '10px', opacity: 0.5, fontFamily: 'var(--font-space)' }} />
                        </Bar>
                    )}
                </BarChart>
            );
    ...
    return (
        ...
        >
            ...
            {(chartType === 'stacked-bar' || chartType === 'stacked-area') && (() => {
                const COMP_COLORS_LEGEND: any = { FixedIncome: '#10b981', Equity: '#3b82f6', RealEstate: '#ef4444', Crypto: '#f59e0b', Pensions: '#8b5cf6' };
                const COMP_LABELS_LEGEND: any = { FixedIncome: 'Fixed Inc', Equity: 'Equity', RealEstate: 'Real Est', Crypto: 'Crypto', Pensions: 'Pensions' };
                const lastDP: any = data.length > 0 ? data[data.length - 1] : {};
