                ...
                <div className="flex-1 flex flex-col gap-4 overflow-y-auto pr-2 pb-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white/70 text-data-sm font-space ">{buyData.broker}</div>
                        </div>
                        <div>
                            <label className="block mb-1 text-white/50 text-xs font-medium uppercase tracking-wider">Currency</label>
                            <div className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white/70 text-data-sm font-space ">{buyData.currency}</div>
                        </div>
                    </div>
        ...
        return (
            <div className="w-full h-full p-8 text-left relative flex flex-col z-10">
                ...
                <p className="mb-6 text-white/60 text-sm">
                    {sellData.broker} · {sellData.sharesHeld.toLocaleString(undefined, { maximumFractionDigits: 4 })} units held · Avg cost: <span className="font-space tabular-nums text-white/80">{formatCurrency(sellData.avgCost, sellData.currency)}</span>
                </p>
                ...
                <div className="flex-1 flex flex-col gap-4 overflow-y-auto pr-2 pb-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block mb-1 text-white/50 text-xs font-medium uppercase tracking-wider">Currency</label>
                            <div className="w-full px-3 py-2.5 bg-white/[0.02] border border-white/5 rounded-xl text-white/50 text-data-sm font-space ">{sellData.currency}</div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-xl p-5 mt-4 flex flex-col gap-4">
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="number" value={sellData.totalProceeds} onChange={e => updateSellCalc('totalProceeds', e.target.value)} step="any"
                                    className="w-full py-2.5 pl-8 pr-3 bg-white/5 border border-white/10 rounded-xl text-white text-lg font-bold focus:outline-none focus:border-rose-500/50 transition-all font-space tabular-nums" />
                            </div>
                        </div>
                        <div className="flex justify-between items-center bg-white/5 rounded-lg p-3">
                            <span className="text-white/60 text-sm">Cost Basis</span>
                            <span className="text-white/80 font-space  text-data-sm">{formatCurrency((sellData.avgCost || 0) * (parseFloat(String(sellData.qtyToSell)) || 0), sellData.currency)}</span>
                        </div>
                        <div className="flex justify-between items-center bg-white/5 border border-white/10 rounded-lg p-3">
                            ...
                            <span className={`text-data-sm font-bold font-space  ${(sellData.pnl || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
