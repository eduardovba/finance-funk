...
function MiniSparklineTooltip({ active, payload, color, currency }: any) {
...
export default function GlowingInvestmentPods({ data, historicalData = [], currency = 'GBP' }: any) {
    if (!data) return null;
    ...
    const pods = [
        {
            ...
            color: '#3b82f6',
            icon: TrendingUp,
            history: historicalData.map((d: any) => ({ value: d.equity || 0, month: d.month }))
        },
        {
            id: 'fixedIncome',
            title: 'FIXED INCOME',
            ...
            color: '#10b981',
            icon: Landmark,
            history: historicalData.map((d: any) => ({ value: d.fixedIncome || 0, month: d.month }))
        },
        {
            id: 'realEstate',
            title: 'REAL ESTATE',
            ...
            color: '#ef4444',
            icon: Building2,
            history: historicalData.map((d: any) => ({ value: d.realEstate || 0, month: d.month }))
        },
        {
            id: 'pensions',
            title: 'PENSIONS',
            ...
            color: '#8b5cf6',
            icon: PiggyBank,
            history: historicalData.map((d: any) => ({ value: d.pensions || 0, month: d.month }))
        },
        {
            id: 'crypto',
            title: 'CRYPTO',
            ...
            color: '#f59e0b',
            icon: Bitcoin,
            history: historicalData.map((d: any) => ({ value: d.crypto || 0, month: d.month }))
        },
        {
            id: 'debt',
            ...
            color: '#ec4899',
            icon: CreditCard,
            history: historicalData.map((d: any) => ({ value: d.debt || 0, month: d.month }))
        }
    ];
                        ...
                        <div className="px-4 pt-4 pointer-events-none relative z-10" style={{ flex: '0 0 auto' }}>
                            <div className="flex items-start justify-between mb-1.5">
                                ...
                                <div className="flex flex-col items-end gap-0.5">
                                    <span className={`text-2xs font-medium px-1.5 py-0.5 rounded shadow-sm flex items-center gap-1 ${isPositive ? 'bg-vu-green/10 text-vu-green' : 'bg-red-500/10 text-red-400'}`}>
                                        <span>{isPositive ? '+' : ''}{pod.diffPct.toFixed(1)}%</span>
                                        <span className="text-2xs opacity-70">MoM</span>
                                    </span>
                                    <span className="text-2xs font-space tabular-nums text-parchment/40">
                                        {isPositive ? '+' : ''}{formatCurrency(pod.diff, currency, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                                    </span>
                                </div>
                            </div>

                            <h3 className="text-2xs uppercase tracking-widest text-[#F5F5DC]/50 font-space mb-0.5">
                                {pod.title}
                            </h3>
