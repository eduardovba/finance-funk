...
export default function GlowingIncomePods({ data, historicalData = [], currency = 'BRL' }: any) {
    if (!data) return null;

    // Convert historical data to specific arrays for recharts
    const salaryHistory = historicalData.map((d: any) => ({ value: d.salary || 0, month: d.month }));
    const realEstateHistory = historicalData.map((d: any) => ({ value: d.realEstate || 0, month: d.month }));
    const equityHistory = historicalData.map((d: any) => ({ value: d.equity || 0, month: d.month }));
    const interestHistory = historicalData.map((d: any) => ({ value: d.fixedIncome || 0, month: d.month }));
                        ...
                        <div className="px-5 pt-5 pointer-events-none relative z-10" style={{ flex: '0 0 auto' }}>
                            ...
                            <div className="flex items-start justify-between mb-2">
                                ...
                                <div className="flex flex-col items-end gap-0.5">
                                    <span className={`text-xs font-medium px-1.5 py-0.5 rounded shadow-sm flex items-center gap-1 ${isPositive ? 'bg-vu-green/10 text-vu-green' : 'bg-red-500/10 text-red-400'}`}>
                            ...
                            <h3 className="text-xs uppercase tracking-widest text-[#F5F5DC]/50 font-space mb-1">
                                {pod.title}
                            </h3>
