...
export default function CryptoForm({
                ...
                <div className="flex flex-col gap-5 flex-1 pb-4">
                    <div className="flex flex-col sm:flex-row gap-4">
                        <div className="flex-1">
                            ...
                            <select value={buyData.broker} onChange={e => setBuyData((prev: any) => ({ ...prev, broker: e.target.value, currency: brokerDict[e.target.value] || 'USD' }))}
    ...
    // Renders inside ContextPane's renderEmptyState
                        ...
                        {[['date', 'Date'], ['asset', 'Asset'], ['broker', 'Platform'], ['investment', 'Total Paid / Received'], ['quantity', 'Quantity'], ['currency', 'Currency'], ['pnl', 'P&L']].map(([field, label]) => (
                            <div key={field}>
                                ...
                                {field === 'currency' ? (
                                    ...
                                    <input type="text" value={editingTr[field] ?? ''} onChange={e => handleEditChange(field, e.target.value)}
