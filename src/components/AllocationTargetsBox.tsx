...
export default function AllocationTargetsBox({ masterMixData, allocationTargets, onTargetsSaved }: { masterMixData: any; allocationTargets: any; onTargetsSaved?: (targets: any) => void; }) {
    ...
    const [editAssetTargets, setEditAssetTargets] = useState<Record<string, number>>({ ...safeAssetTargets });
    const [editCurrencyTargets, setEditCurrencyTargets] = useState<Record<string, number>>({ ...safeCurrencyTargets });
    const [isSaving, setIsSaving] = useState(false);
    const [statusModal, setStatusModal] = useState<{isOpen: boolean, title: string, message: string, type: "success" | "error"}>({ isOpen: false, title: '', message: '', type: 'success' });
    ...
    const handleAssetTargetChange = (key: string, val: string) => {
        let num = parseFloat(val);
        if (isNaN(num)) num = 0;
        setEditAssetTargets(prev => ({ ...prev, [key]: num }));
    };

    const handleCurrencyTargetChange = (key: string, val: string) => {
        let num = parseFloat(val);
        if (isNaN(num)) num = 0;
    ...
    const calculateDeviation = (faderList: any[]) => faderList.reduce((acc, f) => acc + Math.abs(f.actual - f.target), 0);
    ...
    const assetPieData = assetFaders.filter(f => f.target > 0).map(f => ({ name: f.label, value: f.target, fill: (ASSET_COLORS as any)[f.id] }));
    const currencyPieData = currencyFaders.filter(f => f.target > 0).map(f => ({ name: f.id, value: f.target, fill: (CURRENCY_COLORS as any)[f.id] }));

    const CustomPieTooltip = ({ active, payload }: any) => {
        if (active && payload && payload.length) {
            const data = payload[0].payload;
            return (
                <div className="bg-black/90 border border-white/10 rounded px-3 py-2 text-data-xs font-space  shadow-xl backdrop-blur-md">
    ...
    const renderInputRow = (fader: any, onChange: (key: string, val: string) => void) => (
        <div key={fader.id} className="flex justify-between items-center py-2.5 border-b border-white/5 last:border-0 group">
            <div className="flex items-center gap-3">
                <div className="p-1.5 rounded-lg text-white/50 bg-white/5 group-hover:bg-white/10 group-hover:text-white/80 transition-colors">
                    <fader.icon size={14} strokeWidth={2} />
                </div>
                <span className="font-space  text-data-sm tracking-wide text-white/80">{fader.label}</span>
            </div>
            <div className="relative flex items-center justify-end">
                <input
                    ...
                    placeholder="0"
                    className="w-16 bg-black/40 border border-white/10 rounded px-2 py-1.5 text-right font-space  text-data-sm text-[#D4AF37] focus:border-[#D4AF37] outline-none transition-colors"
                />
                <span className="ml-1.5 text-white/40 font-space  text-data-xs">%</span>
            </div>
        </div>
    );

    const renderDriftRow = (fader: any) => {
            ...
            <div key={`drift-${fader.id}`} className="py-3 border-b border-white/5 last:border-0 group">
                <div className="flex justify-between items-end mb-2">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-space  text-data-xs text-white/70 uppercase tracking-widest">{fader.label}</span>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="flex flex-col items-end">
                            <span className="text-2xs text-white/30 uppercase tracking-widest">Target</span>
                            <span className="font-space  text-data-xs text-white/80">{fader.target.toFixed(1)}%</span>
                        </div>
                        <div className="flex flex-col items-end">
                            <span className="text-2xs text-white/30 uppercase tracking-widest">Actual</span>
                            <span className="font-space  text-data-xs text-[#D4AF37] font-bold">{fader.actual.toFixed(1)}%</span>
                        </div>
                        <div className="w-16 text-right">
                            <span className={`font-space  text-data-xs font-bold ${textColor}`}>
                ...
                <div className="mt-1.5 flex justify-end">
                    <span className="text-data-xs font-space  text-white/40">
    ...
    return (
        ...
        <div className="mb-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                ...
                <div className="hidden sm:block">
                    ...
                    >
                        ...
                        <span className="relative font-space  font-bold tracking-[0.1em] text-[#D4AF37] group-disabled:text-white/30 text-data-sm">
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-2 px-1">
                                <span className="text-data-xs font-space  uppercase text-white/50 tracking-widest">Asset Classes</span>
                                <div className={`px-1.5 py-0.5 rounded text-2xs font-space tabular-nums tracking-wider ${isValidAssets ? 'text-emerald-400 bg-emerald-400/10' : 'text-red-400 bg-red-400/10'}`}>
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-2 px-1">
                                <span className="text-data-xs font-space  uppercase text-white/50 tracking-widest">Currency Exposure</span>
                                <div className={`px-1.5 py-0.5 rounded text-2xs font-space tabular-nums tracking-wider ${isValidCurrencies ? 'text-emerald-400 bg-emerald-400/10' : 'text-red-400 bg-red-400/10'}`}>
                ...
                <div id="ftue-alloc-drift" className="rounded-2xl bg-[#121418]/60 backdrop-blur-xl border border-white/[0.06] shadow-[0_8px_32px_rgba(0,0,0,0.4)] p-6 lg:p-8 flex flex-col gap-6 lg:gap-8">
                    <div className="flex justify-between items-center border-b border-white/5 pb-3">
                        ...
                        <div className="flex gap-3">
                            <span className="font-space  text-data-xs tracking-wide text-white/40">
                    ...
                    <div className="flex flex-col gap-6">
                        {/* Asset Class Drift */}
                        <div>
                            <span className="text-data-xs font-space  uppercase text-white/50 tracking-widest block mb-2 px-1">Asset Misalignment</span>
                            <div className="px-1">
                        ...
                        {/* Currency Drift */}
                        <div>
                            <span className="text-data-xs font-space  uppercase text-white/50 tracking-widest block mb-2 px-1">Currency Misalignment</span>
                            <div className="px-1">
            ...
            <PageTutorialOverlay pageId="allocation-targets" steps={ALLOCATION_TUTORIAL_STEPS as any} />
        </>
    );
}
