...
export default function SyncReviewModal({ isOpen, onClose, assets, institutionName, onComplete }) {
    ...
    const handleCategoryChange = async (assetId, newCategory) => {
        setPendingAssets(prev => prev.map(a =>
            a.id === assetId ? { ...a, category: newCategory } : a
        ));
    };

    const handleIgnore = (assetId) => {
    ...
    const handleConfirmAll = async () => {
        ...
        try {
            ...
            await batchVerifyAssets(pendingAssets.map(a => a.id));
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm font-space">
            <motion.div
                ...
                className="bg-[#0F1115] border border-white/10 rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl"
            >
                {/* Header */}
