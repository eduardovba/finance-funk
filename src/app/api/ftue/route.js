...
async function detectCompletedItems(userId) {
    ...
    try {
        ...
        const currPrefs = await kvGet('currency_preferences', null, userId);
        ...
        const forecastSettings = await kvGet('forecast_settings', null, userId);
        ...
        const assetCount = await query('SELECT COUNT(*) as cnt FROM assets WHERE user_id = ?', [userId]);
        if (assetCount?.[0]?.cnt > 0) {
            detected.addFirstHolding = true;
        }

        // 5. recordFirstSnapshot — check snapshots table
        const snapshotCount = await query('SELECT COUNT(*) as cnt FROM snapshots WHERE user_id = ?', [userId]);
        if (snapshotCount?.[0]?.cnt > 0) {
            detected.recordFirstSnapshot = true;
        }

        // 6. setTargets — check allocation targets
        const allocTargets = await kvGet('allocation_targets', null, userId);
...
export async function GET() {
    try {
        ...
        const mergedItems = { ...state.checklistItems };
        for (const [key, val] of Object.entries(detected)) {
            if (val) mergedItems[key] = true;
        }
