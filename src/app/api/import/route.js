...
async function findOrCreateAsset(name, ticker, broker, assetClass, currency, userId, results) {
...
async function checkDuplicate(date, assetId, amount, userId) {
...
async function importEquity(tx, userId, defaultCurrency, defaultBroker, results) {
...
async function importCrypto(tx, userId, defaultCurrency, defaultBroker, results) {
    const currency = tx.currency || defaultCurrency || 'USD';
    const broker = tx.broker || defaultBroker || 'Manual';
    const assetName = tx.asset || tx.ticker || 'Unknown';
...
async function importFixedIncome(tx, userId, defaultCurrency, defaultBroker, results) {
...
async function importPension(tx, userId, defaultCurrency, defaultBroker, results) {
    const broker = tx.broker || defaultBroker || 'Manual';
...
async function importRealEstate(tx, userId, defaultCurrency, defaultBroker, results) {
...
async function importDebt(tx, userId, defaultCurrency, results) {
    const currency = tx.currency || defaultCurrency || 'BRL';
