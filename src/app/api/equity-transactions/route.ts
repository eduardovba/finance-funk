...
type PutEquityBody = z.infer<typeof PutEquitySchema>;
...
export async function GET(): Promise<NextResponse> {
    try {
        ...
        const rows = await query<EquityRow>(sql, [user.id]);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
    try {
        ...
        const { data, error } = validateBody<PostEquityBody>(PostEquitySchema, body);
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
    try {
        const user = await requireAuth();
        const body: unknown = await request.json();
        const { data, error } = validateBody<PutEquityBody>(PutEquitySchema, body);
        if (error) return NextResponse.json({ error }, { status: 400 });
        ...
        await run(
            ...
            [data!.date, dbType, data!.quantity, amount, data!.costPerShare ?? null, data!.pnl ?? null, data!.roiPercent ?? null, data!.currency || 'USD', data!.isSalaryContribution ? 1 : 0, data!.id, user.id]
        );
