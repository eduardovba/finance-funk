...
export default function PlanningError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
    // Auto-retry once on context-related errors (stale cache / transition race)
    useEffect(() => {
        const isContextError =
            error.message?.includes('PortfolioProvider') ||
            error.message?.includes('usePortfolio') ||
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 text-center">
            ...
            >
                ...
                <p className="text-sm text-[#F5F5DC]/60 font-space mb-6">
                    Something went wrong loading the planning page. Retrying…
                </p>
                <button
                    onClick={reset}
